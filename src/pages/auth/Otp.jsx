...
export default function Otp() {
  ...
  // userId y tipo vienen del estado de navegación desde Login
  const { userId, tipo } = location.state || {}
  ...
  // Si no hay userId/tipo, redirigir al login
  if (!userId || !tipo) {
  ...
  async function handleVerify(e) {
    ...
    try {
      const res = await fetch(`${API}/api/auth/verificar-otp`, {
        ...
        body:    JSON.stringify({ userId, otp: code, tipo }),
        credentials: 'include',
      })
      ...
      // Guardar token y redirigir según tipo
      saveToken(data.accessToken)
      navigate(tipo === 'admin' ? '/admin' : '/dashboard', { replace: true })
  ...
  async function handleResend() {
    // Solo navegar de vuelta al login — el usuario debe volver a ingresar credenciales
    navigate('/login')
  }
      ...
      <div style={{ width:'100%', maxWidth:400, textAlign:'center' }}>
        ...
        <div style={{ background:'#fff', borderRadius:20, padding:'36px 32px 28px', boxShadow:'0 2px 24px rgba(0,0,0,0.07)' }}>
          ...
          <p style={{ fontSize:13, color:'#9ca3af', marginBottom:32, lineHeight:1.5 }}>
            Ingresa el código de 6 dígitos que enviamos<br />a tu correo. Expira en 10 minutos.
          </p>
